...
const PaymentManagementSection = forwardRef(({ companyId, validationResult }, ref) => {
    ...
    },
    {
      ...
      body: (rowData) => {
        const isHighlighted = validationResult.some(
          (item) => parseInt(item.vendorPaymentId) === parseInt(rowData.VendorPaymentId) && parseInt(item.rowId) === parseInt(rowData.RowId)
        );
        ...
        return (
          <Text size="textmd" as="p" className="text-[14px] font-normal text-black-900" style={bodyStyle} > 
            {rowData.Invoice}
          </Text >
        );
      }
