...
const Table = forwardRef(
  (
    ...
    const cellEditor = (options) => {
      ...
      // if (!isEditing && !(options.rowData.VendorId === 0)) {
      //   return options.column.props.body(options.rowData);
      // }
        ...
        >
          {expandedColumns.map((columnProps, index) => (
            <Column
              ...
              editor={(options) => {
                if (
                  columnProps.field === "Payee" &&
                  options.rowData.VendorId === 0 // Check if VendorId is 0
                ) {
                  const hasChildRows = (
                    groupedData[options.rowData.VendorPaymentId] || []
                  ).some((row) => row.RowId !== options.rowData.RowId);
                  ...
                  const { VendorPaymentDetailId, VendorPaymentId } =
                    options.rowData;
                  return (
                    <GroupSelect
                      size="sm"
                      ...
                      onChange={(selected) => {
                        ...
                        if (updatedGroupedData[paymentId]) {
                          // Find the index of the current row in the grouped data
                          const rowIndex = updatedGroupedData[
                            paymentId
                          ].findIndex(
                            (row) => row.RowId === options.rowData.RowId
                          );
                          ...
                          if (rowIndex !== -1) {
                            // Update the row in groupedData
                            updatedGroupedData[paymentId][rowIndex] =
                              updatedData;
                          }
                        }
                        ...
                        // Log to verify updated grouped data
                        console.log(
                          "Updated Grouped Data:",
                          updatedGroupedData
                        );

                        // Now, update the localData state to reflect the changes
                        const updatedLocalData = Object.entries(
                          updatedGroupedData
                        ).map(([paymentId, rows]) => {
                          const parentRow = rows.reduce((minRow, currentRow) =>
                            currentRow.RowId < minRow.RowId
                              ? currentRow
                              : minRow
                          );
                          ...
                          return {
                            ...displayParentRow,
                            originalClassName:
                              fieldMaps.classNameMap[paymentId],
                            originalGLAccount:
                              fieldMaps.glAccountMap[paymentId],
                            originalInvoiceDate:
                              fieldMaps.invoiceDateMap[paymentId],
                            originalInvoice: fieldMaps.invoiceMap[paymentId],
                            originalMemo: fieldMaps.memoMap[paymentId],
                          };
                        });
