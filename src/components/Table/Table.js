...
const Table = forwardRef(({
    ...
    if (col.field === 'paymentMethodHeader') {
      return {
        ...
        body: (rowData) => {
          ...
          const FilterchildRows = childRows.filter(row => row.RowId !== rowData.RowId);
          const hasChildRows = childRows.length > 0;
  ...
  const rowExpansionTemplate = (data) => {
    ...
    const emptyRowExists = rows.some(row => row.RowId === emptyChildRow.RowId);
            ...
            {enhancedColumns.map((col, colIndex) => (
              ...
              >
                {col.field === "Payee" ? (
                  ...
                  col.field === "InvoiceDate" || col.field === "Invoice" ||
                  col.field === "Memo" ? (
                  editingCell === `${childRow.RowId}-${col.field}` ? (
                    <input
                      type="text"
  ...
  function saveDataWithChildren(localData) {
    ...
    localData.forEach(row => {
        ...
        const changedChildRows = childRows.filter(childRow => 
            childRow.Change === 1 && childRow.RowId !== row.RowId
        );
  ...
  const savevendorPayment = async () => {
    ...
    processedData.forEach((val) => {
      ...
      const childRow = groupedData[val.VendorPaymentId] || [];
        const childRows = childRow.filter(
          (row) => row.RowId !== val.RowId
      );
     const hasChildRows =  childRows.length > 0;
